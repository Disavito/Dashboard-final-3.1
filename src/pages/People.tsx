...
interface SocioTitular extends SocioTitularBase {
  ...
  netIncomeAmount: number; // Nuevo: para almacenar el monto neto de ingresos
}
...
function People() {
  ...
  const fetchSocios = useCallback(async () => {
    ...
    const enrichedSocios: SocioTitular[] = (sociosData || []).map(socio => {
      ...
      return { ...socio, isActive, receiptNumber, netIncomeAmount }; // Add netIncomeAmount
    });
  ...
  const exportToCsv = (data: SocioTitular[], filename: string) => {
    ...
    const headers = [
      ...
      'Distrito Vivienda',
      'Dirección Vivienda',
      'Monto Neto de Ingresos', // Nuevo campo para exportación
    ];
    ...
    data.forEach(socio => {
      const row = [
        ...
        `"${socio.netIncomeAmount.toFixed(2) || '0.00'}"`, // Exportar el monto neto
      ];
